...
describe('GreetingService', () => {
  ...
  test('should greet valid names', () => {
    expect(greetingService.greet('John')).toBe(
      'Hello, John! Welcome to Test App'
    );
    expect(greetingService.greet('Alice')).toBe(
  ...
  test.each([
    ...
    (name: string | number | null | undefined, expectedError: string) => {
      expect(() => greetingService.greet(name as string)).toThrow(
        expectedError
      );
    }
  );
  ...
  test('should handle edge cases with valid characters', () => {
    expect(greetingService.greet("O'Connor")).toBe(
      "Hello, O'Connor! Welcome to Test App"
    );
    expect(greetingService.greet('Jean-Paul')).toBe(
      'Hello, Jean-Paul! Welcome to Test App'
    );
    expect(greetingService.greet('Mary Jane')).toBe(
      'Hello, Mary Jane! Welcome to Test App'
    );
  });
  ...
  test('should throw error when config is null', () => {
    expect(
      () => new GreetingService(null as unknown as AppConfig, logger)
    ).toThrow('Configuration is required for GreetingService');
  });

  test('should throw error when logger is null', () => {
    expect(
      () => new GreetingService(config, null as unknown as Logger)
    ).toThrow('Logger is required for GreetingService');
  });
});
...
describe('AppInfoService', () => {
  ...
  test('should throw error when config is null', () => {
    expect(
      () => new AppInfoService(null as unknown as AppConfig, logger)
    ).toThrow('Configuration is required for AppInfoService');
  });
...
describe('initialize TS', () => {
  ...
  test('should initialize application successfully', async () => {
    ...
    expect(mockConsole.log).toHaveBeenCalledWith(
      '🚀 Starting TypeScript application...'
    );
    ...
    expect(mockConsole.log).toHaveBeenCalledWith('\n📝 Example Usage:');
    expect(mockConsole.log).toHaveBeenCalledWith(
      'Hello, Developer! Welcome to Test App'
    );
    expect(mockConsole.log).toHaveBeenCalledWith(
      'Hello, TypeScript User! Welcome to Test App'
    );
    expect(mockConsole.log).toHaveBeenCalledWith('\n👥 Multiple Greetings:');
    expect(mockConsole.log).toHaveBeenCalledWith(
      '  Hello, Alice! Welcome to Test App'
    ...
    expect(mockConsole.log).toHaveBeenCalledWith(
      '  Hello, Charlie! Welcome to Test App'
    );
    expect(mockConsole.log).toHaveBeenCalledWith('\n📊 Application Info:');
