...
wsl -d kali-linux -- bash -c "cd '%WSL_DIR%' && echo '🐧 Welcome to WSL (Kali Linux)!' && echo '📁 Successfully entered project directory' && echo '🔧 Environment will load automatically via direnv' && echo '' && echo '💡 Available commands:' && echo '  • ./scripts/setup-dev-workspace.sh  - Create development workspace' && echo '  • npm run dev                        - Start development server' && echo '  • npm test                          - Run tests' && echo '  • docker compose up -d              - Start services' && echo '' && echo '🎯 Your development environment is ready!' && echo '' && echo '🔄 Setting up development workspace...' && (if [ -f './scripts/setup-dev-workspace.sh' ]; then ./scripts/setup-dev-workspace.sh; else echo '⚠️  Workspace setup script not found. Run manually: ./scripts/setup-dev-workspace.sh'; fi) && echo '📂 Returning to project directory...' && cd '%WSL_DIR%' && exec bash --rcfile <(echo 'cd '\''%WSL_DIR%'\''; source ~/.bashrc')"
