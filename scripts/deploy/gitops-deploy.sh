...
load_configuration() {
    ...
    local config_file="config/gitops.config"
...
generate_deployment_manifest() {
    ...
    sed -e "s/{{ENVIRONMENT}}/$ENVIRONMENT/g" \
        ...
        "scripts/manifests/deployment.yaml" > "$manifest_file"
}
...
generate_service_manifest() {
    ...
    sed -e "s/{{ENVIRONMENT}}/$ENVIRONMENT/g" \
        ...
        "scripts/manifests/service.yaml" > "$manifest_file"
}
...
generate_ingress_manifest() {
    ...
    sed -e "s/{{ENVIRONMENT}}/$ENVIRONMENT/g" \
        ...
        "scripts/manifests/ingress.yaml" > "$manifest_file"
}
